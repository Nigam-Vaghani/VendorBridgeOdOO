...
import { getDashboardAnalytics } from '../../api/procurementApi';
...
export const Dashboard = () => {
  ...
  const statCards = data ? [
    { title: "Active RFQ's", value: data.active_rfqs.toString(), icon: FileText },
    { title: "Pending Approvals", value: data.pending_approvals.toString(), icon: Clock },
    { title: "PO's this month", value: formatCurrency(data.po_this_month), icon: DollarSign },
    { title: "overdue invoices", value: data.overdue_invoices.toString(), icon: AlertCircle },
  ] : [];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-white/20 dark:divide-white/10">
                  {data.recent_purchases.length === 0 ? (
                    <tr><td colSpan="4" className="px-6 py-4 text-center text-foreground/50">No recent purchases found.</td></tr>
                  ) : data.recent_purchases.map((purchase, index) => (
                    <tr key={index} className="hover:bg-white/30 dark:hover:bg-black/30 transition-colors">
                      ...
                      <td className="px-6 py-4 text-foreground/80 font-mono">{formatCurrency(purchase.amount)}</td>
          ...
          <div className="flex flex-wrap gap-4 pt-4 border-t border-white/20 dark:border-white/10">
            <button onClick={() => navigate('/rfqs')} className="flex items-center gap-2 px-6 py-3 bg-white/40 dark:bg-black/40 backdrop-blur-md border border-white/30 dark:border-white/10 text-primary rounded-lg font-semibold hover:bg-primary hover:text-primary-foreground transition-all shadow-[0_4px_20px_rgb(0,0,0,0.05)] hover:-translate-y-0.5">
              <Plus size={18} />
              new RFQ
            </button>
            ...
            <button onClick={() => navigate('/purchase-orders')} className="flex items-center gap-2 px-6 py-3 bg-white/40 dark:bg-black/40 backdrop-blur-md border border-white/30 dark:border-white/10 text-primary rounded-lg font-semibold hover:bg-primary hover:text-primary-foreground transition-all shadow-[0_4px_20px_rgb(0,0,0,0.05)] hover:-translate-y-0.5">
              <FilePlus size={18} />
              Create Invoices
            </button>
          </div>
